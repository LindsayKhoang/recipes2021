...
def parser_array_of_str(error_message):
    return [
        eval,
        lambda tab: isinstance(tab, list)
...
environment_variables = LazyDict(
    {
        ...
        "ALLOWED_HOSTS": {
            ...
            "parser": parser_array_of_str(
                _("ALLOWED_HOSTS = list of str only!")
            ),
        },
        "INTERNAL_IPS": {
            "default": '["127.0.0.1", ]',
            "required": True,
            "parser": parser_array_of_str(
                _("INTERNAL_IPS = list of str only!")
            ),
        },
...
def parse_var(var, infos):
    ...
    if "parser" in infos:
        ...
        if var not in settings:
            raise Exception(
                f"{var}: variable not set in environment, and "
